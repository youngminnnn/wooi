...
export function registerIpc(ctx: IpcContext): void {
  ...
  const restackWholeStack = async (
    ...
    for (const entry of stack) {
      ...
      if ((await detectRemoteDivergence(worktreePath, entry.branch)) === 'diverged') {
        emitStep(divergedStep(entry.branch, entry.prNumber))
        return {
          status: 'error',
    ...
    try {
      ...
      if (ws.stack && ws.stack.length > 1) {
        ...
        if ((await detectRemoteDivergence(ws.worktreePath, ws.branch)) === 'diverged') {
          const step = { ...divergedStep(ws.branch, ws.prNumber), workspaceId }
          progress.step(step)
  ...
  const runMergeCascade = async (
    ...
    for (const child of children) {
      ...
      if (remote === 'diverged') {
        const step = { ...divergedStep(child.branch, child.prNumber), workspaceId: child.id }
