...
export function registerIpc(ctx: IpcContext): void {
  ...
  /**
   * 위임 대상 백엔드 변경(실험 기능). 세션 옵션에 실리는 값이라 **다음 세션부터** 적용된다 —
   * 여기서 세션을 끊지 않는 이유는, 도는 중인 턴을 설정 변경만으로 죽이는 편이 더 놀랍기 때문이다.
   */
  ipcMain.handle(
    IPC.workspaceSetSubBackends,
    (_e, workspaceId: string, subBackends: AgentBackendId[]) => {
      store.update((st) => {
        const w = st.workspaces.find((x) => x.id === workspaceId)
        if (w) w.multiAgent = multiAgentConfigFrom(subBackends, w.agentBackend)
      })
      broadcastState()
    }
  )
