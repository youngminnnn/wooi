...
export async function moveCommitDown(opts: {
  ...
  // `pushed:false` 라도 error 가 없으면 리모트에 그 브랜치가 아직 없다는 뜻이다(git.ts) — 아직
  // PR 을 열지 않은 층이라 push 할 곳이 없는 것이고, 실패가 아니다. 그걸 실패로 세면 로컬에만 있는
  // 스택에서 이동이 항상 error 로 끝난다. 진짜 실패는 사유를 그대로 실어 보낸다 — 삼키면 리모트가
  // 옛 커밋에 멈춘 채 다음 restack 이 그걸 갈라짐으로 오진한다(#430 이 restack 에서 겪은 그 문제다).
  const lowerPush = await pushForceWithLease(lower.worktreePath, lower.branch)
  if (lowerPush.error) {
    return {
      status: 'error',
      failedStep: 'push-lower',
      message: `The local move succeeded, but the lower branch could not be force-pushed: ${lowerPush.error}`,
      before,
  ...
  const upperPush = await pushForceWithLease(upper.worktreePath, upper.branch)
  if (upperPush.error) {
    return {
      status: 'error',
      failedStep: 'push-upper',
      message:
        'The lower branch was pushed, but the upper branch was not, so the upper pull request ' +
        `temporarily shows the moved commit twice: ${upperPush.error}`,
      before,
      after: await tips(affected),
      rolledBack: false,
      steps
