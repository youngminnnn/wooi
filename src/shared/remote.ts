...
export interface RemoteWorkspace {
  ...
  /**
   * 미확인 완료 — 응답이 끝났는데 아직 아무 화면에서도 열어 보지 않았다. 데스크톱 사이드바의
   * 파란 점과 **같은 값**이다(랩탑 렌더러가 소유하고, main 을 거쳐 여기 실린다).
   *
   * 폰이 따로 세지 않고 랩탑 것을 미러하는 이유는 읽음이 이미 양방향이기 때문이다 — 폰이
   * 워크스페이스를 열면 `remote:watch` 가 랩탑의 미확인 표시를 지운다(`evt:remoteRead`).
   * 폰만 따로 세면 랩탑에서 이미 확인한 것이 폰에서는 영영 안 읽음으로 남아 한쪽만 지워진다.
   *
