 ...
 * 명령이 그 턴으로 빨려 들어가(두 백엔드 모두 mid-turn 입력을 받아들인다) 엉뚱한 답을 받는다.
 * 실제로 그랬다: 인수인계 턴에 "요약만 하고 기다려라" 라고 시켜 뒀으니, 그 사이 들어온 사용자
 * 명령은 무시되고 요약만 돌아왔다. 다음 메시지에 얹으면 턴은 하나뿐이고, 사용자가 이어서 일하지
 * 않으면 비용도 없다.
 *
