...
**다만 하나는 먼저 고쳐야 한다.** §2.1 은 도입 여부와 무관하게 *지금* 존재하는 결함을 기록한다.
체인이 GitHub 스택이면 머지 시점에 GitHub 이 서버에서 캐스케이딩 rebase 하는데, 그 뒤 Wooi 의
캐스케이드가 그 위로 force-push 해서 위 레이어가 자기만의 diff 를 잃는다. 바깥에서 만들어진
스택을 흡수하면 도달하고, 그 흡수는 `buildStackFromPrs` 가 이미 한다. 그래서 갈라짐 가드는 어떤
연동 작업보다 앞선 §8 의 1단계다.

아래 주장은 이 문서 작성자가 2026-08-11~12 에 `gh stack` v0.1.0 과 실제 GitHub API 로 직접
재현한 것은 **[확인됨]**, 문서만 근거인 것은 **[미확인]** 으로 표시한다.

§2.1 의 머지 시점 동작은 2026-08-12 에 스크래치 리포
(`youngminnnn/stacked-pr-playground`)에서 실제로 실행해 확인했다. `gh stack link` 로 3층 스택을
만들고 맨 아래 PR 을 squash 머지한 뒤, 그 결과의 ref·PR base·리뷰 코멘트 앵커, 그리고 Wooi 자신의
force-push 경로를 직접 관찰했다. 그 브랜치와 PR(#46–48, 스택 #49)은 다시 확인할 수 있도록
그대로 남겨 뒀다.
...
- `--open` 없이 만들면 PR 이 **draft** 로 생기고, `gh stack merge` 는 draft 를 거부한다
  (`pull request #46 is a draft; mark it ready for review before merging`). Wooi 가 PR 없는
  브랜치에 `link` 를 걸게 된다면 이 기본값을 물려받지 말고 draft 여부를 의식적으로 정해야 한다.
  **[확인됨]**
...
**GitHub 은 원격 ref 를 실제로 다시 쓴다.** 스크래치 리포
(`youngminnnn/stacked-pr-playground`, 스택 #49 / PR #46–48, 3층 `q1/1-a` → `q1/2-b` →
`q1/3-c`)에서 처음부터 끝까지 재현했다. 맨 아래 PR 을 squash 머지하니 `main` 이 `d5208b1` 로
움직이면서 위 두 브랜치가 함께 다시 쓰였다. **[확인됨]**

| ref | 머지 전 | 머지 후 |
|---|---|---|
| `main` | `f71c618` | `d5208b1` |
| `q1/1-a` (머지됨) | `2e7b5a0` | `2e7b5a0` |
| `q1/2-b` | `504d24f` | **`8ed4598`** |
| `q1/3-c` | `b5bbdd8` | **`09b733b`** |

squash 커밋이 다시 쓰인 두 브랜치의 조상이 됐고, `q1/2-b` 의 PR base 는 `q1/1-a` → `main` 으로
옮겨졌다. GitHub 은 로컬 개입 없이 체인 전체를 서버에서 올바르게 캐스케이딩 rebase 한다.
**[확인됨]**

이걸로 질문은 끝났고, 답은 나쁜 쪽이다. 게다가 위험한 경우가 예외가 아니라 **기본 경로**다.
`restackOnto`(`src/main/git.ts:533`)가 이렇게 쓰여 있기 때문이다.

1. `restackOnto` 는 push 전에 스스로 `fetchRemote()` 를 부른다(`git.ts:547`). 그래서 push 시점엔
   `origin/<branch>` 가 GitHub 이 다시 쓴 sha 로 갱신돼 있고 **lease 가 다시 유효해진다.**
   lease 는 이걸 막지 못한다.
2. 캐스케이드 경로는 항상 `oldBase` 를 넘기고(`ipc.ts:1372`),
   `needsRebase = oldBase ? true : behind > 0`(`git.ts:556`)이므로 **무조건 true** 다. 브랜치가
   이미 새 base 를 담고 있어도 Wooi 는 rebase 한다.
3. 로컬 worktree 는 rebase 이전 커밋을 그대로 들고 있으므로, Wooi 는 그것을 `origin/main` 위로
   재생해 **내용은 같은데 sha 만 다른** 커밋을 만든다.

그대로 재현했다. 로컬 `q1/2-b` 를 옛 `504d24f` 에 두고 fetch → `git rebase --onto origin/main
q1/1-a` → `git push --force-with-lease` 하니
`+ 8ed4598...58c8440 q1/2-b -> q1/2-b (forced update)`. **push 가 성공하고 GitHub 의 rebase 를
덮어쓴다.** **[확인됨]**

(참고로 추적 ref 가 낡았을 때는 lease 가 실제로 push 를 거부한다 —
`! [rejected] q1/2-b -> q1/2-b (stale info)` **[확인됨]**. 하지만 `restackOnto` 는 항상 먼저
fetch 하므로 Wooi 는 그 갈래에 도달하지 않는다.)

**구체적인 피해는 위 레이어가 자기만의 diff 를 잃는 것이다.** GitHub 만 rebase 한 상태에서
PR #48 은 파일 하나(`q1-c.txt`)만 보여 줬다 — 스택의 존재 이유 그 자체다. Wooi 의 force-push 가
그 base 를 밑에서 갈아치운 뒤 #48 은 둘(`q1-b.txt`, `q1-c.txt`)을 보여 준다. head 가 이제
고아가 된 `8ed4598` 의 후손이기 때문이다. **[확인됨]** 맨 위 레이어의 리뷰어가 아래 레이어까지
보게 된다.

반대로 위험이 아닌 것으로 밝혀진 것이 둘 있다. 리뷰 코멘트는 고아가 되지 않고 **다시
앵커링된다** — GitHub 이 `commit_id` 를 새 sha 로 갱신하면서 `original_commit_id` 를 보존하고,
GitHub 자신의 rebase 와 Wooi 의 외부 force-push 양쪽에서 그렇게 했다. **[확인됨]** 그리고 스택
객체 자체는 덮어쓰기 뒤에도 온전하다(엔트리 3개, position 정상). **[확인됨]**

순수한 경합도 있다. Wooi 의 캐스케이드는 머지를 *감지*(PR 상태 폴링)해서 도는데, GitHub 의
rebase 는 머지 그 자체가 방아쇠다. Wooi 는 반드시 늦게 도착하고, GitHub 이 이미 고쳐 놓은
상태에 대고 동작한다.

**이건 무언가를 도입해서 생기는 위험이 아니라 지금 있는 위험이다.** 스택을 발행하지 않아도
걸린다. `buildStackFromPrs` 가 이미 열린 PR 체인을 흡수하므로, `gh stack` 으로 스택을 만든 뒤
Wooi 에서 여는 사용자는 Wooi 가 캐스케이드로 망가뜨릴 체인을 그대로 갖게 된다. 그래서 갈라짐
가드(§3.3)는 개선이 아니라 **선결 조건**이다 — §8 을 보라.
...
게이팅은 완전히 알려져 있지 않지만 걱정보다는 좁다. 체인지로그는 "앞으로 며칠에 걸쳐 모든
리포지터리에 public preview 로 롤아웃"이라 하고 플랜이나 조직 제한을 언급하지 않으며 **[문서로
확인됨]**, 평범한 **private 개인 리포**에서 스택 생성·link·머지까지 전 과정이 exit 9 없이
동작했다 **[확인됨]**. 즉 공개 범위나 조직 소유 여부로 게이팅되지 않는다. 그래도 확장에는
명시적인 "이 리포에서 활성화되지 않음" 오류와 전용 종료 코드가 있으니 어떤 형태로든 리포별
게이팅이 존재한다. **[확인됨]** exit 9 를 근거로 삼고 사용 가능하다고 가정하지 않는다.

식별자를 파싱할 사람을 위한 참고 하나. 스택 번호와 PR 번호는 번호 공간을 공유한다. 실험한
스택은 가장 최근 PR 이 **#48** 인 리포에서 **#49** 였다. **[확인됨]** 확장 문서가 이 성질에
기대고 있으므로("stack and PR numbers never overlap"), 숫자 하나는 종류 사이에서만 모호하고 같은
종류 안에서는 모호하지 않다.

## 7. 남은 질문

1. ~~서버 측 스택 머지가 그 위 브랜치들의 원격 ref 를 다시 쓰는가?~~ **답: 그렇다.** §2.1 에서
   재현했다 — GitHub 이 체인 전체를 서버에서 캐스케이딩 rebase 하고, Wooi 의 캐스케이드가 그걸
   덮어써서 위 레이어가 자기만의 diff 를 잃는다.
2. ~~리뷰 코멘트는 다시 걸리는가, 미아가 되는가?~~ **답: 다시 걸린다.** `commit_id` 가 새 sha 로
   갱신되고 `original_commit_id` 는 보존되며, GitHub 자신의 rebase 와 외부 force-push 양쪽에서
   그렇다. 코멘트 유실은 위험 목록에서 뺀다.
3. exit 9 는 정확히 무엇을 기준으로 하는가 — 리포, 조직, 플랜, 롤아웃 코호트? 일부 좁혀졌다.
   **private 개인 리포**에서 exit 9 없이 스택이 동작했으므로, 공개 범위나 조직 소유 여부가
   기준은 아니다. **[확인됨]**
4. §2.1 의 갈라짐을 감지했을 때 Wooi 는 실제로 **무엇을 해야 하는가?** 단계로 기록하고 사용자에게
   알리는 것이 안전한 하한이지만, 더 나은 답은 로컬 브랜치를 GitHub 이 다시 쓴 ref 로
   fast-forward 하고 rebase 를 아예 건너뛰는 것일 수 있다 — GitHub 이 이미 올바르게 해 놨다.
   다만 별도 설계가 필요하다. worktree 가 dirty 할 수 있고, 그 브랜치가 체크아웃돼 있을 수 있고,
   에이전트가 세션 중일 수 있다.
5. 이미 base 가 올바르게 연결된 PR 들에 `gh stack link` 를 걸면 PR 타임라인에 소음으로 남는
   무의미한 base "교정"이 발생하는가?
6. PR 을 연 계정과 `link` 를 실행하는 계정이 다를 때도 동작하는가? Wooi 는 연결된 사용자로 PR 을
   열므로 아마 무의미한 질문이지만 확인되지 않았다.
7. 스택 생성을 위한 문서화된 정식 REST/GraphQL mutation 이 있어서 발행에서 확장 의존성을 아예
   뗄 수 있는가? 지금 알려진 것은 확장의 비문서화 `POST /repos/{o}/{r}/stacks` 뿐이다.

## 8. 구현 순서

질문 1 에 답이 나오면서(§2.1) 순서가 바뀌었다. 갈라짐 가드는 원래 3번이었고 "그 자체로 가치가
있다" 정도였는데, 이제 **1번이자 핵심**이다. Wooi 가 아무것도 도입하지 않은 지금도 GitHub 스택
체인을 망가뜨리기 때문이다.

1. **갈라짐 가드.** 자식을 rebase 하기 전에 원격 ref 와 로컬 tip 을 비교하고, Wooi 가 만들지 않은
   방식으로 원격이 움직였으면 rebase 대신 새 `StackCascadeStep` 상태를 기록해
   `StackSyncBanner` 에 띄운다. §2.1 의 덮어쓰기를 막는 수정이다. **lease 에 기댈 수 없다는 점이
   중요하다** — `restackOnto` 가 push 전에 fetch 하므로 그 시점엔 lease 가 항상 유효하다(§2.1).
   검사는 명시적이어야 한다.
2. **읽기, 확장 없이.** `getRepoStacks` / `getStackForPr` 를 담은 `ghStack.ts` 와 2단계 감지.
   `StackPopover` 에 스택 번호와 GitHub 순서를 노출. 쓰기 위험 0, 새 의존성 0. 1번에 값싸고
   정확한 방아쇠도 준다 — PR 이 GitHub 스택에 속하면 GitHub 이 rebase 했다고 보면 된다.
3. **질문 4 를 결정한다** — 보고만 할지, GitHub 의 rebase 위로 fast-forward 할지. 설계 후 구현.
...
1·2 단계는 preview 가 어떻게 되든 할 가치가 있다. 특히 1 번은 살아 있는 결함을 고치는 일이다.
4·5 단계는 웹 UI 스택 맵을 원하는 구체적인 사용자가 있는 게 아니라면 preview 가 preview 를
벗어날 때까지 기다리는 편이 좋다.
