...
- Screen-reader announcements duplicate visible text into the DOM. `LiveRegion.tsx` mirrors turn transitions, pending questions, errors, and every toast into `sr-only` live regions, and `sr-only` is clipped rather than hidden, so Playwright still matches it. A bare `getByText('…')` on an announced string resolves to two elements and fails on strict mode. Scope the lookup to the surface you mean — `[data-toast]` for toasts, `[data-live-region="turn"|"toast"|"alert"]` for what is spoken. Two specs had to be repaired for this.
- Do not select toasts by `role="alert"`. Toasts carry `data-toast`; the assertive live region deliberately carries no ARIA role, because a permanently mounted empty `role="alert"` would be counted as a live alert and would never detach for a `waitFor({ state: 'detached' })`.
