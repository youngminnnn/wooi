...
**병렬로 돌리는 건 어렵지 않습니다. 어려운 건 머지까지입니다.**
...
에이전트를 나란히 돌리는 건 **가로축**이고, 격리된 worktree 가 이미 그 몫을 합니다. **worktree 는
파일 충돌은 막아도 의존성은 못 풉니다** — 2번 작업이 1번의 스키마 변경 위에서 이어져야 할 때,
격리가 오히려 걸림돌입니다. Wooi 는 여기에 **세로축**을 더합니다. 이어지는 작업은 자기가 딛고
선 작업의 브랜치에서 분기하고, 부모가 병합돼도 Wooi 가 체인 전체를 유효하게 유지합니다. 그
체인은 에이전트가 내장 도구로 직접 쌓을 수도 있고, 리뷰는 읽고 마는 기록이 아니라 diff 위에
붙습니다.

> **에이전트 지원** — v1.4.0부터 **Claude Code**(Claude Agent SDK 경유)와
> **OpenAI Codex**(Codex CLI 경유)를 모두 지원합니다. 워크스페이스를 만들 때 에이전트를
> 선택하고, 첫 메시지를 보내기 전까지는 바꿀 수 있습니다. 그 뒤로는 해당 워크스페이스가
> 선택한 에이전트를 유지합니다.

## 왜 Wooi 인가

- 🧱 **Stacked PR 이 1급 시민입니다** — 지금 서 있는 브랜치뿐 아니라 앱 전체가 "체인"을 압니다.
  앞선 작업 위에서 이어지는 작업은 기본 브랜치가 아니라 그 작업의 브랜치에서 분기합니다.
  **Restack** 은 최신 부모 위로 rebase 하고 `--force-with-lease` 로 push 합니다. 부모가 병합되면
  **병합 캐스케이드**가 자식 PR 의 base 를 조부모로 옮기고 자식들을 그 위로 rebase 합니다. base 가
  엉뚱하게 어긋난 PR 은 **조용히 받아들이는 대신 알려 줍니다.** 에이전트가 `git checkout -b` 와
  `gh pr create` 로 손수 만든 체인도 **PR 의 base 링크에서 스택을 복원**해 똑같이 보여 줍니다.
- 🤖 **에이전트가 리포가 아니라 앱을 조종합니다** — 모든 세션에 내장 MCP 서버가 붙습니다.
  `check_related_work` 로 다른 워크스페이스가 건드리는 파일을 **시작하기 전에** 확인하고,
  `create_workspace`·`create_stacked_workspace` 로 작업을 리뷰 가능한 PR 체인으로 쪼개며,
  `report_to_parent`·`notify_child` 로 결과와 소식을 체인을 따라 주고받습니다. 울타리도 있습니다.
  에이전트는 자기가 만든 워크스페이스만 겨냥할 수 있고, 형제와 공유되는 건 **파일 경로뿐 — diff 는
  절대 넘기지 않습니다.** 프롬프트 하나가 직접 짜지 않은 3단 PR 스택이 되기도 합니다.
  [내장 MCP 레퍼런스](docs/built-in-mcp.ko.md)를 참고하세요.
- 🔍 **리뷰는 한 문단이 아니라 하나의 작업 공간입니다** — PR 리뷰는 이제 어떤 에이전트든 합니다.
  없는 건 그 결과를 다룰 자리입니다. Wooi 의 리뷰는 **전용 worktree** 를 가진 독립 엔티티라,
  지금 작업 중인 체크아웃을 건드리지 않고 변경된 hunk 바깥까지 읽습니다. 지적은 **diff 줄에
  앵커**되어 심각도 배지와 함께 붙고, 고치고 버리고 낱개로든 한꺼번에든 게시합니다. 파일에는
  **봤음** 표시를 달 수 있는데, 새 커밋이 그 파일을 바꾸면 저절로 풀립니다. 게시한 코멘트의
  답글은 주기적으로 가져와 이어서 물어볼 수 있고, 판정에는 가드가 있습니다 — 내 PR 에서는
  approve 를 숨기고, 근거가 될 지적이 게시에 실패하면 판정을 보류합니다.
- 🎛️ **두 백엔드를 얕지 않게** — Claude Code 와 OpenAI Codex 를 최소 공통분모로 깎아내는 대신
  각자의 세부까지 따라갑니다. 백엔드가 지원할 때의 `/rewind`·`/btw`, 백엔드별 권한 모드,
  reasoning effort 와 fast mode, 상태줄 하나로 정규화해 보여 주는 계정 레이트리밋, 그리고
  사용자가 CLI 에 설정해 둔 MCP 서버 승계까지. [에이전트 백엔드](docs/agent-backends.ko.md)를
  참고하세요.
- 🔒 **조용해야 할 곳에서 조용합니다** — **텔레메트리 없음**, 자체 서버 없음, 대화 기록은 로컬에만
  남습니다. 업데이트는 돌아가는 턴을 끊는 대신 **"작업이 다 끝나면 재시작"** 을 기다릴 수 있고,
  `.env` 처럼 git 이 추적하지 않지만 꼭 필요한 파일은 **새 worktree 마다 전달**됩니다. 세션은
  앱을 껐다 켜도 복원됩니다.

다른 도구와 어떻게 다른지 궁금하시다면, 근거와 확인 날짜를 함께 적어 둔
[비교 페이지](https://youngminnnn.github.io/wooi/alternatives.html)를 참고해 주세요.
...
무엇을 먼저 누르게 되는지가 아니라, Wooi 를 Wooi 답게 만드는 순서로 적었습니다.
...
설정도 필요 없습니다. 에이전트는 자기 worktree 의 파일만 고치는 게 아니라 Wooi 자체를 조종합니다.
작업을 쪼개고, 조각마다 워크스페이스를 열고, 의존 순서대로 쌓고, 결과를 체인을 따라 넘깁니다.

- **옆자리부터 확인** — `check_related_work` 는 이 리포에 열려 있는 다른 워크스페이스와 각각이
  바꾸고 있는 파일 경로를 알려 주고, 이 에이전트가 건드리려는 경로와 겹치는 부분을 짚어 "그 경로를
  고치기 전에 사용자에게 먼저 말하라"고 안내합니다. 넘어가는 건 경로뿐 — **diff 는 워크스페이스
  사이를 절대 넘지 않습니다.**
- **펼치거나 쌓거나** — `create_workspace` 는 기본 브랜치에서 독립 워크스페이스를,
...
- **체인을 따라 주고받기** — `report_to_parent` 는 자기가 딛고 선 워크스페이스에 결과를
  기록하고(부모의 턴을 끊지 않습니다), `check_stacked_work` 는 자식들의 브랜치·PR 상태·최신
  보고를 나열하며, `notify_child` 는 자식에게 소식을 보내 턴을 시작합니다(바쁘면 현재 턴 뒤에
  큐잉됩니다).
- **일상적인 도구도** — `open_pull_request`(base 는 Wooi 가 고릅니다 — 스택이면 부모 브랜치,
  아니면 기본 브랜치), `list_issues`, `archive_workspace`, 그리고 리포에 설정해 둔 스크립트를
  다루는 `run_script`·`stop_script`·`read_script_output`.
- **울타리** — 대부분의 도구는 호출한 워크스페이스에만 작용합니다. `workspaceId` 를 받는 도구는
  호출자가 **직접 만든** 워크스페이스만 겨냥할 수 있고, `archive_workspace` 는 자기 자신을
...
- **큰 diff 에서 자리 잃지 않기** — 파일에 **봤음** 표시를 하면 접혀 비켜 줍니다. 이 표시는 그
  파일 내용의 지문과 함께 저장되므로, 새 커밋이 그 파일을 바꾸면 저절로 안 본 상태로 돌아가고
...
Wooi 는 코딩 에이전트 두 종류 — **Claude Code**(Claude Agent SDK) 와 **OpenAI Codex**(Codex
CLI) — 를 지원하되, 둘을 최소 공통분모로 합치는 대신 각자의 조작 방식을 그대로 따라갑니다.
렌더러가 선택된 백엔드의 능력을 읽으므로, 워크스페이스에는 그 백엔드에서 실제로 동작하는
컨트롤만 뜹니다.
...
- **Shift+Tab 으로 순환하는 권한 모드** — Claude Code 는 default·accept edits·plan·auto 를,
  Codex 는 read only·auto·full access·plan 을 제공하고 현재 모드는 입력창 아래에 표시됩니다.
  권한 프롬프트는 Allow/Deny 외에 **"Always allow"**(이 세션 동안 해당 도구 자동 허용)를
  제공합니다 — Enter=Allow / Esc=Deny.
- **한쪽에만 있는 기능** — `/rewind`(파일 체크포인트로 코드 되돌리기)와 `/btw`(턴을 흐트러뜨리지
  않고 곁가지 질문)는 백엔드가 지원할 때 나타나고, 턴 중간 steering 도 되는 쪽에서는 큐잉 대신
  그대로 씁니다.
- **fast mode 와 effort** — 상태줄에서 고르거나 `/model`·`/effort`·`/fast` 로 워크스페이스마다
  지정합니다. 지금 모델이 지원하지 않는 선택지는 조용히 무시하지 않고 그렇다고 표시합니다.
...
- 워크스페이스를 만들 때 **Claude Code 또는 Codex를 선택**하고, 첫 메시지를 보내기 전까지는
  `/agent` 로 바꿀 수 있습니다. 그 뒤로는 그 에이전트를 유지하며, 스택으로 만든 워크스페이스는
  부모의 에이전트를 물려받습니다.
...
- **파일 전달(carry)** — 새 worktree 에는 git 이 추적하는 파일만 들어오므로 무시되는 파일
