...
격리된 worktree 에서 에이전트를 돌리는 건 이제 기본입니다. 안 풀린 쪽은 그 다음입니다.
**에이전트를 하나 더 돌릴 때마다 rebase 할 브랜치와 리뷰를 기다리는 PR 이 하나씩 늘어납니다.**
Wooi 는 이어지는 작업을 쌓아 애초에 부딪히지 않게 하고, 리뷰는 읽고 마는 기록이 아니라 그 위에서
작업하는 diff 로 만듭니다.
...
- 🧱 **이어지는 작업은 충돌하지 않고 쌓입니다** — 다른 도구에서 병렬 브랜치 충돌은 터진 뒤에
  수습합니다. Wooi 의 답은 구조적입니다. 에이전트 워크스페이스를 다른 워크스페이스의 브랜치 위에
  쌓고, 부모가 병합되면 Wooi 가 자식들을 rebase 하고 PR base 를 옮깁니다. 남은 스택은 충돌 더미가
  되는 대신 그대로 유효합니다.
