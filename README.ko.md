...
- 🧱 **스택의 단위가 에이전트** — 에이전트 워크스페이스를 다른 워크스페이스의 브랜치 위에 쌓습니다.
  부모가 병합되면 Wooi 가 자식들을 rebase 하고 PR base 를 옮깁니다. 남은 스택은 그대로 유효.
- 🔍 **PR 리뷰를 diff 위에서** — PR 리뷰는 이제 어떤 에이전트든 합니다. 없는 건 그 결과를 다룰
  화면입니다. 지적이 해당 diff 줄 위에 바로 붙고, 하나씩 고치고 버리고, 낱개로든 한꺼번에든
  게시합니다.
