...
- 🧱 **Stacked PR 이 곁다리가 아니라 기본입니다** — 지금 서 있는 브랜치뿐 아니라 앱 전체가
  ...
  `git checkout -b` 와 `gh pr create` 로 손수 만든 체인도 **PR 의 base 링크에서 스택을 복원**해
  똑같이 보여 줍니다.
- 🤖 **에이전트가 앱을 조종합니다** — 모든 세션에 내장 MCP 서버가 붙습니다.
  ...
  에이전트는 자기가 만든 워크스페이스만 겨냥할 수 있습니다. 형제와 공유되는 건 **파일
  경로뿐입니다. diff 는 절대 넘기지 않습니다.** 프롬프트 하나가 직접 짜지 않은 3단 PR 스택이
  되기도 합니다. [내장 MCP 레퍼런스](docs/built-in-mcp.ko.md)를 참고하세요.
- 🔍 **리뷰 자체가 하나의 작업 공간입니다** — PR 리뷰는 이제 어떤 에이전트든 합니다. 없는 건 그
