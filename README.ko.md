...
- 🔍 **PR 리뷰를 diff 위에서** — PR 리뷰는 이제 어떤 에이전트든 합니다. 없는 건 그 결과를 다룰
  화면입니다. 지적이 해당하는 diff 줄 위에 바로 붙고, 하나씩 고치고 버리고, 낱개로든 한꺼번에든
  게시합니다.
...
Wooi 는 대신 diff 자체를 작업대로 만듭니다. 그리고 리뷰할 PR 은 대개 동료의 것이므로, 리뷰는 내
작업의 다음 단계가 아닙니다 — 사이드바에 자기 행을 갖고, 워크스페이스 하나를 차지하는 대신 그
옆에서 나란히 돕니다.
