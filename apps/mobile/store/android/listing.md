...
**예.** 앱 안에서는 설정 → "Unpair this phone" 이 이 폰의 접근과 릴레이의 기기 행을 없애고,
맥에서는 설정 → Remote → "Delete all remote data" 가 릴레이에 남은 것을 전부 지운다.

**웹 삭제 요청 URL** 칸에는 이것을 넣는다:

```
https://youngminnnn.github.io/wooi/privacy.html#data-deletion-requests
```

Play 는 "앱에서 계정을 만들 수 있으면 웹 삭제 요청 URL 이 필수" 라고 본다. Wooi 는 사용자가
계정을 만들지 않지만 페어링 과정에서 Supabase 익명 인증으로 기기 행이 생기는 회색지대라,
`PRIVACY.md` 에 [Data deletion requests](../../../../PRIVACY.md) 절을 만들고 그 앵커를 낸다.
**App Store Connect 에도 같은 URL 을 낸다**([`../ios/listing.md`](../ios/listing.md)) — 같은
사실에 두 개의 절차가 있으면 어느 쪽이 진짜인지 다투게 된다.

그 절이 앱 안 경로와 이메일 경로를 함께 적어 두는 이유는, 앱을 이미 지운 사람에게는 앱 안
버튼이 답이 되지 못하기 때문이다. 릴레이의 행은 익명이라 이메일 요청만으로 특정하지 못할 수도
있는데, 그 한계도 그 절에 적혀 있다 — 폼이 요구하는 것은 "삭제해 준다는 주장" 이 아니라
**실제 절차**다.
