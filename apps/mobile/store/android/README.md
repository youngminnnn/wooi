...
| 파일 | 규격 | 상태 |
| --- | --- | --- |
| `icon-512.png` | 512×512, **32비트 PNG(알파 포함)**, 최대 1MB | ✅ `src/icon.svg` 에서 생성 |
| `feature-graphic.png` | 1024×500, **24비트 PNG(알파 없음)** | ✅ `src/feature-graphic.svg` 에서 생성 |
| `screenshots/*.png` | 1080×1920, 폰 기준 최소 2장·최대 8장 | ✅ 4장 — 에뮬레이터에서 촬영 |
...
빠진다. `src/icon.svg` 가 `docs/icon.svg` 와 달리 `rx` 와 가장자리 하이라이트를 빼고 꽉 찬
...
찍을 화면 4장(대형 화면 추천 노출 조건이 4장 이상이다):

1. 워크스페이스 목록 — 여러 세션이 병렬로 도는 모습
2. 트랜스크립트 — 에이전트가 실제로 무엇을 하는지
3. 권한 승인 — 이 앱의 존재 이유에 가장 가까운 화면
4. 페어링 QR — 랩탑과 어떻게 이어지는지
